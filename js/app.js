(() => {
  ...
  const getNextToPublish = () => getAllEpisodes().filter((item) => item.editingStatus !== "PUBLICADO" && item.publishDate >= data.trip.currentDate).sort((a,b) => a.publishDate.localeCompare(b.publishDate))[0];
  ...
  const applyCurrentLocation = (position) => {
    ...
    if(!match || match.distance>75 || match.city===data.trip.currentCity)return;
    data.trip.currentCity=match.city; state.weatherCity=null; render();
  };
  ...
  const sidebarTimeReference = () => {
    const city=weatherLocations[data.trip.currentCity] ? data.trip.currentCity : "La Paz";
  ...
  const pageTitle = (title, subtitle) => `<div class="page-top"><div>${state.navigationDepth>0?`<button class="button ghost nav-back" data-nav-back>← Voltar</button>`:""}<div class="eyebrow">${data.trip.name}</div><h1>${title}</h1>${subtitle?`<div class="subtle">${subtitle}</div>`:""}</div>${button("Atualizar","data-action=refresh","secondary")}</div>`;
  ...
  const routeCard = (item) => { const isOpen = state.expanded === item.id; const ep = episode(item.episode); return `<div class="timeline-card" data-route-card="${item.id}"><div class="list-item"><button class="item-toggle" data-expand="${item.id}"><span class="item-row"><span><span class="item-title">${dateText(item.date)} · ${item.city}</span><span class="item-meta">${item.activity}</span></span><span class="tag">${item.dayType}</span></span></button>${isOpen?`<div class="details"><p><b>Local:</b> ${item.place || item.destination || item.city}</p>${ep?`<p><b>Episódio ${ep.number}:</b> ${ep.title}</p>`:""}${item.mode?`<p><b>Deslocamento:</b> ${item.origin} → ${item.destination} · ${item.time}</p>`:""}<p><b>Sequência:</b> ${item.points.join(" → ")}</p><div class="actions">${button("Abrir no Maps",mapSearch(item.place || item.destination || item.city),"small")}${item.origin&&item.destination?button("Como chegar",mapDirections(item.origin,item.destination,item.mode==="caminhada"?"walking":"driving"),"small"):""}${item.lodgingId?button("Voltar à hospedagem",mapSearch(lodging(item.lodgingId).place),"small"):""}</div></div>`:""}</div></div>`; };
  const todayView = () => { const today=data.itinerary.find(i=>i.date===data.trip.currentDate); const next=data.itinerary.find(i=>i.date>data.trip.currentDate && i.mode); const nextRecording=data.episodes.find(e=>e.recordingDate>=data.trip.currentDate); const nextPublication=data.episodes.find(e=>e.publishDate>=data.trip.currentDate); const currentLodging=lodging(data.trip.activeLodgingId); const nextLabel=data.trip.currentCity==="La Paz"?"Simular chegada em Coroico":"Voltar à simulação de La Paz"; const cityLabel=tripProgress().phase==="before"?"Primeira cidade":"Cidade atual"; return `${pageTitle("Hoje",`<span data-trip-progress>${tripProgressText()}</span>`)}<section class="hero"><div class="eyebrow" data-trip-city-label>${cityLabel}</div><h2>${data.trip.currentCity}</h2><p>${today.dayType}<br />${today.activity}</p><div class="actions">${button(nextLabel,"data-action=simulate-city","small")}</div></section><section class="route-card"><div class="route-top"><div><div class="eyebrow">Próxima mudança de cidade</div><div class="route-line">${next.origin} <span>→</span> ${next.destination}</div><div class="subtle">${formatDate(next.date,{weekday:"long",day:"2-digit",month:"long"})} · ${next.time} · ${next.duration || "3h30"}</div></div><span class="status warning">PENDENTE</span></div><div class="actions wide-actions">${button("Rota",mapDirections(next.origin,next.destination),"small")}${button("Hospedagem",mapSearch(lodging(next.lodgingId).place),"small")}</div></section><div class="grid two">${metric("Próxima gravação",formatDate(nextRecording.recordingDate),nextRecording.title)}${metric("Próxima publicação",formatDate(nextPublication.publishDate),nextPublication.title)}${metric("Próximo prazo","Hoje","Transfer para Coroico")}${metric("Edição","Em curso","Vídeo do domingo")}</div><div class="content-columns"><div><section class="section"><div class="section-heading"><h2>Próximos 7 dias</h2><button class="button ghost" data-open-route="week">Ver roteiro</button></div><div class="list">${data.itinerary.filter(i=>i.date>=data.trip.currentDate).slice(0,7).map(i=>`<button class="list-item" data-open-route="week" data-route-item="${i.id}"><div class="item-row"><div><span class="item-title">${formatDate(i.date)} · ${i.city}</span><div class="item-meta">${i.activity}</div></div><span class="tag">${i.dayType}</span></div></button>`).join("")}</div></section></div><div><section class="section"><div class="section-heading"><h2>Alertas importantes</h2><button class="button ghost" data-view="pending">Ver tudo</button></div>${alertList()}</section><section class="section"><h2>Base atual</h2><div class="list-item"><b>${currentLodging.name}</b><div class="item-meta">${currentLodging.address} · checkout ${formatDate(currentLodging.checkout)}</div><div class="actions">${button("Voltar para hospedagem",mapSearch(currentLodging.place),"small")}</div></div></section></div></div>`; };
  const routeView = () => { let items=[...data.itinerary]; const today=data.trip.currentDate; if(state.routeFilter==="today") items=items.filter(i=>i.date===today); if(state.routeFilter==="week") items=items.filter(i=>i.date>=today).slice(0,7); if(state.routeCity!=="all") items=items.filter(i=>i.city===state.routeCity); const cities=[...new Set(data.itinerary.map(i=>i.city))]; return `${pageTitle("Roteiro","Sequência operacional da viagem") }<div class="chip-bar"><button class="chip ${state.routeFilter==="today"?"active":""}" data-route-filter="today">Hoje</button><button class="chip ${state.routeFilter==="week"?"active":""}" data-route-filter="week">Próximos 7 dias</button><button class="chip ${state.routeFilter==="all"?"active":""}" data-route-filter="all">Viagem completa</button></div><div class="filter-row"><select id="city-filter"><option value="all">Todas as cidades</option>${cities.map(c=>`<option value="${c}" ${state.routeCity===c?"selected":""}>${c}</option>`).join("")}</select></div><div class="timeline">${items.length?items.map(routeCard).join(""):`<div class="empty">Nenhum dia corresponde a este filtro.</div>`}</div>`; };
  const pendingView = () => { const filters=["all","VENCIDA","URGENTE","VENCE HOJE","PRÓXIMA","NO PRAZO","SEM PRAZO","BLOQUEADA"]; const tasks=state.taskFilter==="all"?data.tasks:data.tasks.filter(t=>t.situation===state.taskFilter); const totals=[["Vencidas","VENCIDA","critical"],["Urgentes","URGENTE","critical"],["Vence hoje","VENCE HOJE","today"],["Próximas","PRÓXIMA","soon"],["No prazo","NO PRAZO","good"],["Sem prazo","SEM PRAZO","no-deadline"],["Bloqueadas","BLOQUEADA","blocked"]]; return `${pageTitle("Pendências","Leitura operacional, sem edição") }<div class="grid desktop-four two">${totals.map(([label,status,tone])=>metric(label,data.tasks.filter(t=>t.situation===status).length,"",tone)).join("")}</div><div class="chip-bar">${filters.map(f=>`<button class="chip ${state.taskFilter===f?"active":""}" data-task-filter="${f}">${f==="all"?"Todas":f}</button>`).join("")}</div><div class="counter">${tasks.length} tarefa${tasks.length!==1?"s":""}</div><section class="section list">${tasks.map(t=>{const tone=statusTone(t.situation);return `<div class="list-item task-card ${tone}"><button class="item-toggle" data-expand="${t.id}"><span class="item-row"><span><span class="item-title">${t.task}</span><span class="item-meta">${t.area} · ${t.owner}${t.due?` · ${formatDate(t.due)}`:""}</span></span><span class="status ${tone}">${t.situation}</span></span></button>${state.expanded===t.id?`<div class="details"><p><b>Prioridade:</b> ${t.priority}</p><p><b>Dependência:</b> ${t.dependency}</p><p>${t.note}</p><div class="actions">${button("Abrir referência",`data-action=mock-link`,"small")}</div></div>`:""}</div>`;}).join("")}</section>`; };
  ...
  const tripSubview = () => { const view=state.tripView; const title={flights:"Voos",transports:"Transportes",lodgings:"Hospedagens",reservations:"Reservas",documents:"Documentos",contacts:"Contatos"}[view]; let content=""; if(view==="flights") content=data.flights.map(flightCard).join(""); if(view==="transports") content=data.transports.map(t=>`<div class="list-item"><div class="item-row"><div><b>${t.origin} → ${t.destination}</b><div class="item-meta">${t.mode} · ${formatDate(t.date)} · ${t.time} · ${t.duration}</div></div><span class="status ${statusTone(t.status)}">${t.status}</span></div><div class="actions">${cityButton(t.origin)}${cityButton(t.destination)}${button("Abrir terminal",mapSearch(t.terminalStart),"small")}${button("Como chegar",mapDirections(t.origin,t.destination),"small")}${button("Ver reserva",`data-action=mock-link`,"small")}${button("Contato",`data-action=mock-link`,"small")}</div></div>`).join(""); if(view==="lodgings") content=data.lodgings.map(h=>`<div class="list-item"><div class="item-row"><div><b>${h.city} · ${h.name}</b><div class="item-meta">${h.address} · check-out ${formatDate(h.checkout)}</div></div><span class="status ${statusTone(h.status)}">${h.status}</span></div><div class="actions">${cityButton(h.city)}${button("Ir para hospedagem",mapSearch(h.place),"small")}${button("Abrir reserva",`data-action=mock-link`,"small")}${button("Contato",`data-action=mock-link`,"small")}</div></div>`).join(""); if(view==="reservations") content=["confirmado","pendente","a reservar"].map(status=>`<section class="section"><h2>${status}</h2><div class="list">${data.reservations.filter(r=>r.status===status).map(r=>`<div class="list-item"><b>${r.item}</b><div class="item-meta">${r.type} · ${r.supplier} · ${formatDate(r.date)}</div><div class="item-meta">${r.note}</div></div>`).join("")||`<div class="empty">Nenhum item.</div>`}</div></section>`).join(""); if(view==="documents") content=`<div class="list">${data.documents.map(d=>`<div class="list-item item-row"><b>${d.name}</b><span class="status ${statusTone(d.status)}">${d.status}</span></div>`).join("")}</div>`; if(view==="contacts") content=`<div class="list">${data.contacts.map(c=>`<div class="list-item"><b>${c.name}</b><div class="item-meta">${c.category} · ${c.note}</div><div class="actions">${button("Ligar",`data-action=mock-link`,"small")}${button("WhatsApp",`data-action=mock-link`,"small")}</div></div>`).join("")}</div>`; return `<div class="subview-top"><button class="button ghost" data-trip-back>←</button><h1>${title}</h1></div>${content}`; };
  const tripView = () => { if(state.tripView) return tripSubview(); const current=lodging(data.trip.activeLodgingId); const entries=[ ["flights","Voos","status e acesso rápido"],["transports","Transportes","deslocamentos e terminais"],["lodgings","Hospedagens","base atual e próximas"],["reservations","Reservas","confirmadas, pendentes e a reservar"],["documents","Documentos","somente status"],["contacts","Contatos","números fictícios do protótipo"] ]; return `${pageTitle("Viagem",`${data.trip.startDate.split("-").reverse().join("/")} → ${data.trip.endDate.split("-").reverse().join("/")} · ${data.trip.generalStatus}`)}<section class="route-card"><div class="eyebrow">Hospedagem atual</div><h2>${current.name}</h2><p class="subtle">${current.address} · check-out ${formatDate(current.checkout)}</p><div class="actions">${cityButton(current.city)}${button("Ir para hospedagem",mapSearch(current.place),"small")}${button("Abrir no Maps",mapSearch(current.place),"small")}</div></section><section class="trip-menu">${entries.map(([id,title,detail])=>`<button class="menu-card" data-trip-view="${id}"><span><b>${title}</b><small>${detail}</small></span><span class="arrow">›</span></button>`).join("")}</section><section class="section"><h2>Checklist de saída</h2><ul class="checklist">${["Backup feito","Baterias carregadas","Equipamentos conferidos","Documentos offline","Transporte confirmado","Check-out","Hospedagem seguinte","Mapa offline","Internet na próxima base"].map((item,index)=>`<li><input class="check" type="checkbox" id="check-${index}" ${index<3?"checked":""}><label for="check-${index}">${item}</label></li>`).join("")}</ul></section><div class="subtle">Última atualização: <b>${data.trip.lastUpdated}</b> · Offline disponível futuramente.</div>`; };
  ...
  const renderPlaceVisual = () => { const hero=$(".hero"); if(!hero || state.view!=="today") return; const current=data.itinerary.find(i=>i.date===data.trip.currentDate); const place=`${current?.place || data.trip.currentCity}, Bolívia`; const visual=cityVisual(data.trip.currentCity); if(!visual) return; hero.insertAdjacentHTML("beforeend",`<div class="place-visual"><img src="${visual.image}" alt="Vista de ${data.trip.currentCity}" /><a class="photo-source" href="${visual.source}" target="_blank" rel="noopener noreferrer">${visual.credit} <span class="external-mark" aria-hidden="true">↗</span></a><a class="map-source" href="${buildGoogleMapsSearchUrl(place)}" target="_blank" rel="noopener noreferrer">Google Maps <span class="external-mark" aria-hidden="true">↗</span></a></div>`); };
  const renderWeatherPanel = async () => {
    ...
    const hero=state.view==="today"?$("#app > .hero"):null; const city=state.weatherCity || data.trip.currentCity; const location=weatherLocations[city]; if(!hero || !location) return;
    ...
    const cities=[...new Set(data.itinerary.filter(item=>item.date>=data.trip.currentDate).map(item=>item.city))].filter(item=>weatherLocations[item]);
    hero.insertAdjacentHTML("afterend",`<section class="weather-forecast"><div class="weather-heading"><div><div class="eyebrow">Clima de referência</div><h2>Próximos 15 dias</h2><p class="weather-caption">Previsão ao vivo para <b>${city}</b>. A data simulada da viagem só terá previsão quando entrar nesta janela.</p></div><div class="weather-cities">${cities.map(item=>`<button class="weather-city ${city===item?"active":""}" data-weather-city="${item}">${item}</button>`).join("")}</div></div><div class="weather-days"><div class="weather-loading">Carregando previsão...</div></div><a class="weather-source" href="https://open-meteo.com/en/docs" target="_blank" rel="noopener noreferrer">Fonte climática: Open-Meteo <span class="external-mark" aria-hidden="true">↗</span></a></section>`);
  ...
  const renderRouteContext = () => {
    ...
    const city=details?selected.city:state.routeCity==="all"?data.trip.currentCity:state.routeCity;
    if(!cityVisual(city))return;
  ...
  const bind = () => { document.querySelectorAll("[data-view]").forEach(el=>el.addEventListener("click",()=>{state.view=el.dataset.view;state.tripView=null;state.expanded=null;render();$("#app").focus();})); document.querySelectorAll("[data-open-route]").forEach(el=>el.addEventListener("click",()=>{state.view="route";state.routeFilter=el.dataset.openRoute;state.tripView=null;state.expanded=null;render();$("#app").focus();})); document.querySelectorAll("[data-route-filter]").forEach(el=>el.addEventListener("click",()=>{state.routeFilter=el.dataset.routeFilter;state.expanded=null;render();})); const city=$("#city-filter"); if(city) city.addEventListener("change",()=>{state.routeCity=city.value;state.expanded=null;render();}); document.querySelectorAll("[data-task-filter]").forEach(el=>el.addEventListener("click",()=>{state.taskFilter=el.dataset.taskFilter;state.expanded=null;render();})); document.querySelectorAll("[data-expand]").forEach(el=>el.addEventListener("click",()=>{state.expanded=state.expanded===el.dataset.expand?null:el.dataset.expand;render();})); document.querySelectorAll("[data-trip-view]").forEach(el=>el.addEventListener("click",()=>{state.tripView=el.dataset.tripView;render();})); const back=$("[data-trip-back]"); if(back)back.addEventListener("click",()=>{state.tripView=null;render();}); document.querySelectorAll("[data-action=refresh]").forEach(el=>el.addEventListener("click",()=>{data.trip.lastUpdated="Agora mesmo";showToast("Dados mockados atualizados localmente.");render();})); document.querySelectorAll("[data-action=simulate-city]").forEach(el=>el.addEventListener("click",()=>{const isLaPaz=data.trip.currentCity==="La Paz"; Object.assign(data.trip,isLaPaz?{currentDate:"2026-11-10",currentCity:"Coroico",nextCity:"La Paz",period:"Dia 7 de 49",activeLodgingId:"h2"}:{currentDate:"2026-11-07",currentCity:"La Paz",nextCity:"Coroico",period:"Dia 4 de 49",activeLodgingId:"h1"}); showToast(isLaPaz?"Chegada em Coroico simulada localmente.":"Retorno a La Paz simulado localmente.");render();})); document.querySelectorAll("[data-action=mock-link]").forEach(el=>el.addEventListener("click",()=>showToast("Ação futura: sem link real neste protótipo."))); };
  render();
})();
