(() => {
  ...
  const bind = () => { document.querySelectorAll("[data-view]").forEach(el=>el.addEventListener("click",()=>{state.view=el.dataset.view;state.tripView=null;state.expanded=null;render();$("#app").focus();})); document.querySelectorAll("[data-open-route]").forEach(el=>el.addEventListener("click",()=>{state.view="route";state.routeFilter=el.dataset.openRoute;state.tripView=null;state.expanded=null;render();$("#app").focus();})); document.querySelectorAll("[data-route-filter]").forEach(el=>el.addEventListener("click",()=>{state.routeFilter=el.dataset.routeFilter;state.expanded=null;render();})); const city=$("#city-filter"); if(city) city.addEventListener("change",()=>{state.routeCity=city.value;state.expanded=null;render();}); document.querySelectorAll("[data-task-filter]").forEach(el=>el.addEventListener("click",()=>{state.taskFilter=el.dataset.taskFilter;state.expanded=null;render();})); document.querySelectorAll("[data-expand]").forEach(el=>el.addEventListener("click",()=>{state.expanded=state.expanded===el.dataset.expand?null:el.dataset.expand;render();})); document.querySelectorAll("[data-trip-view]").forEach(el=>el.addEventListener("click",()=>{state.tripView=el.dataset.tripView;render();})); const back=$("[data-trip-back]"); if(back)back.addEventListener("click",()=>{state.tripView=null;render();}); document.querySelectorAll("[data-action=refresh]").forEach(el=>el.addEventListener("click",()=>{data.trip.lastUpdated="Agora mesmo";showToast("Dados mockados atualizados localmente.");render();})); document.querySelectorAll("[data-action=mock-link]").forEach(el=>el.addEventListener("click",()=>showToast("Ação futura: sem link real neste protótipo."))); };
  render();
})();
